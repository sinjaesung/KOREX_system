...
export default function JoinTab() {
  ...
  console.log('component>member/company>joinselect 컴포넌트 실행=====================');
  ...
  const [isceo, setIsceo] = useState(undefined);//대표인가 여부 (기업회원)

  const [nextshow, setNextShow] = useState(false);//처음엔 아무것도 없다가, 인증발송 요청시에 두next중 하나는 나온 이후시점부터 true값 지님

  const [active, setActive] = useState(false);
  const [active2, setActive2] = useState(false);//active2상태여야 최종적 다음단계 넘어갈수있다.
  ...
  const cernumChange = (e) => {
    setCernum(e.target.value);
  }

  const aligoSmsSend = async (e) => {
    console.log('aligosmsSend발송함수 호출:', name, phone, cernum);
    ...
    if (check_id) {
      switch (check_id) {
        case 'check1':
          ...
          document.getElementById('CompanyJoinInfo_move').style.display = 'block';
          document.getElementById('CompanyJoinAgree_move').style.display = 'none';
          break;
  ...
  const is_company_ceo_changes = (e) => {
    ...
    if (check_id) {
      if (nextshow) {
        switch (check_id) {
          ...
          case 'check2':
            //사업자 대표가 아니라면 대표 대표아닌 여부 상관없이 사업자정보등록페이지 넘김
            document.getElementById('CompanyJoinInfo_move').style.display = 'block';
            document.getElementById('CompanyJoinAgree_move').style.display = 'none';
            setIsceo(false);
            break;
  ...
  useEffect(() => {
    console.log('useEffect상태값변화(기업가입):', name, phone, cernum, verify_cernum, isceo);
    if (checkVaildate())
      setActive(true);
    else
      setActive(false);
  ...
  const nextStep = async (e) => {
    ...
    if (cernumchk_validate_result) {
      ...
      if (cernumchk_validate_result.success) {
        ...
        history.push('/CompanyJoinInfo');
      } else {
        //실패인 경우
        //alert(cernumchk_validate_result.message);//인증벉호다르거나,서버오류거나,유효시간 지난경우에 한함.
        setActive2(false);
  ...
  return (
    <>
      <Wrapper>
        ...
        <div className="par-spacing">
          <WrapChooseBox>
            <FormControl component="fieldset">
              <RadioGroup >
                <FormControlLabel value="check1" control={<Radio id="check1" className="is_company_ceo" onChange={is_company_ceo_changes} />} label="본인은 사업자 대표입니다." />
                <FormControlLabel value="check2" control={<Radio id="check2" className="is_company_ceo" onChange={is_company_ceo_changes} />} label="본인은 사업자 대표가 아닙니다." />
              </RadioGroup>
            </FormControl>

            {/* <ChooseBox>
          <Checkbox type="radio" name="company" id="check1" className="is_company_ceo" onChange={is_company_ceo_changes} />
        ...
        <ChooseBox>
          <Checkbox type="radio" name="company" id="check2" className="is_company_ceo" onChange={is_company_ceo_changes} />
        ...
        <WrapJoinInput>
            ...
            <div className="par-spacing-before" id='CompanyJoinAgree_move' style={{ display: "none" }}>
            <MUButton_Validation_2 variant="contained" type="submit" name="" active2={active2} onClick={nextStep}>
              <Link to="/CompanyJoinAgree" className="data_link" />
              다음
            </MUButton_Validation_2>
            </div>
            {/*  2) 사업자 대표 선택시 ( 사업자 정보 등록페이지로 넘어간다 ) */}
            {/* <Link to="/CompanyJoinInfo">
            <GoNextPage type="submit" name="" active2={active2} className='nextButtons' id='CompanyJoinInfo_move' style={{ display: "none" }} onClick={nextStep}>다음</GoNextPage>
          </Link> */}
            
            <div className="par-spacing-before" id='CompanyJoinInfo_move' style={{ display: "none" }}>
            <MUButton_Validation_2 variant="contained" type="submit" name="" active2={active2} onClick={nextStep}>
              <Link to="/CompanyJoinInfo" className="data_link" />
              다음
            </MUButton_Validation_2>
            </div>
            ...
            <Wrap_MUButton_R31>
              <MUButton_R31>
                <Link to="/CompanyLogin" className="data_link" />
                로그인
              </MUButton_R31>
              <MUButton_R31>
