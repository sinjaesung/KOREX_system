  ...
  const checkVaildate = () =>{
   ...
   const nextStep = async (e) => {
     ...
     if(active){
       ...
       //xxx-xx-xxxxx형태 문자열인가 적절여부 검사 기업,분양대행사 회원또한 사업자등록번호 유효성검사여부 클라이언트검사(홈택스인증)added
       var crnumber_sendinfo = {
         crNumber : business_number
       };
       console.log('crnumber validation send data:',JSON.stringify(crnumber_sendinfo));
       var cr_validation_result = await serverController.connectFetchController('/api/auth/company/crNumber_validation','POST',JSON.stringify(crnumber_sendinfo));
       console.log('cr_validationresultss::',cr_validation_result);
    ...
    return (
        <Container>
          ...
          <WrapJoinInput>
            ...
            <RegistInputArea>
            <TextField type="text" name="" onChange={regiChange1} />
            <Dash>-</Dash>
            <TextField type="text" name="" onChange={regiChange2} />
            <Dash>-</Dash>
            <TextField type="text" name="" onChange=
              {regiChange3} />
            </RegistInputArea>
              ...
              <Dash>-</Dash>
              <RegistInput type="text" name="" onChange=
              {regiChange3}/> */}
            ...
            <SubmitButton>
              ...
              <MUButton variant="contained" type="submit" name="" active={active}>다음</MUButton>
              </Link>
            </SubmitButton>
            {/* <SubmitButton>
              <Link to="/CompanyJoinAgree" onClick={nextStep}> 
