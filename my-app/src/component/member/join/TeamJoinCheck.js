...
//redux데이터 접근 은 page외부 companymemjoinagree에서가 아닌 최종적으론 여기다 company>joincheck에서 한다. submit시에 보내기
import { useSelector } from 'react-redux';
import {tempRegisterUserdataActions } from '../../../store/actionCreators';

export default function JoinSns({active, setActive, agreeStaus,setAgreeStatus, setAgreePossible,agreePossible,team_regist_submit_function}) {
    //전체 선택 체크박스 ! !
    const [allCheck, setAllCheck] = useState(false);

    const tempregisteruserdata= useSelector(data => data.temp_register_userdata);
    console.log('company>joincheck element요소 실행 함수 실행===================');
    ...
    return (
      ...
      {/*전체 체크 항목*/}
        ...
        {/*가입 버튼*/}
          <JoinBtn type="submit" name="" active={active} onClick={team_regist_submit_function}>가입</JoinBtn>
        </Container>
  );
}
