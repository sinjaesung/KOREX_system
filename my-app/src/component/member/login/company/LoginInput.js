...
export default function JoinInput() {
  ...
  const company_login_submit = async (e) => {
    ...
    if (active == true) {
      ...
      console.log('company login res results:', res);
      //alert(res);

      if (!res.success) {
        document.getElementById('loginfail').style.display = 'block';

        /*if(res.message.indexOf && res.message.indexOf('팀원초대미수락')!=-1){
    ...
    <>
      <Wrapper>
        ...
        <div className="par-spacing">
          <MUTextFiled label="휴대전화" type="text" helperText="휴대번호를 '-' 빼고 입력해주세요." onChange={phoneChange} />
        </div>
        <div className="par-spacing">
          <MUTextFiled label='비밀번호' type="password" onChange={pwdChange} />
        </div>
        ...
        <div className="par-spacing-before">
          <MUButton_Validation variant="contained" type="submit" name="" active={active} onClick={company_login_submit}>로그인</MUButton_Validation>
        </div>
        <div className="par-spacing-after">
          <Wrap_MUButton_R31>
            <MUButton_R31>
              <Link to="/CompanyJoin" className="data_link" />
              회원가입
            </MUButton_R31>
            <MUButton_R31>
...
const MUTextFiled = styled(TextField)`
