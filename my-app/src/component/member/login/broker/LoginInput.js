...
export default function JoinInput() {
    ...
    if (active) {
      ...
      if (!res.success) {
        ...
        /*if(res.message.indexOf('팀원초대미수락')!=-1){
          //로그인 자체 성공했으나 초대미수락 회원이여서 페이지이동처리(팀원초댐수락권유페이지) 해주는 분기작성
  ...
  return (
    <>
      <Wrapper>
        <div className="par-spacing"><p>대표외 소속팀원은 팀원추가 후 로그인 가능합니다.</p></div>
        ...
        <div className="par-spacing-before">
          <MUButton_Validation variant="contained" type="submit" name="" active={active} onClick={broker_login_submit}>로그인</MUButton_Validation>
        </div>
        <div className="par-spacing-after">
          <Wrap_MUButton_R31>
            <MUButton_R31>
              <Link to="/BrokerJoin" className="data_link" />
              회원가입
            </MUButton_R31>
            <MUButton_R31>
