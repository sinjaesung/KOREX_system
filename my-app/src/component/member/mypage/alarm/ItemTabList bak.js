...
export default function ItemTabList({value,setexculsive_supply_privacycompany_notilist,setexculsive_supply_probroker_notilist,setexculsive_demand_notilist,setexculsive_supply_privacycompany_notilist_notseencnt,setexculsive_supply_probroker_notilist_notseencnt,setexculsive_demand_notilist_notseencnt, setbunyangsuyo_notilist, setbunyangsupply_notilist,setbunyangsuyo_notilist_notseencnt,setbunyangsupply_notilist_notseencnt}) {
  ...
  console.log('itemTablist valuesss허허허:',value);
  ...
  switch(value.noti_type){
    ...
    case '6':
      alram_case ='거래개시승인요청결과';//전속매물공급(중개사)
    break;
  ...
  const showModal2 =()=>{
    ...
    return (
        <Container>
            <TabContent>
              <WrapAlarmInfo>
                <Condition>상태:<Orange>{value.noti_status==0?'안읽음':'읽음'} {value.noti_status}</Orange></Condition>
                <FlexBox>
                  <Left>알림일시 noti_id:({value.noti_id})</Left>
                  <Right>{value.reserv_plan_date?value.reserv_plan_date:value.modify_date}</Right>
                </FlexBox>
                <FlexBox>
                  <Left>알림케이스 명</Left>
                  <Right>{alram_case}</Right>
                </FlexBox>
                <FlexBox>
                  <Left>사건 ID</Left>
                  <Right>{value.txn_id}</Right>
                </FlexBox>
                <FlexBox>
                  <Left>내용</Left>
                  <RightWd100 onClick={ async() => {
