...
export default function SearchApartOfficetel({ selectInfo, setSelectInfo, activeIndex, overdos }) {
  ...
  const searchWord = async (e) => {
    ...
    // console.log('정보확인 : ',);
    console.log('정보확인666 : ', res_result.data.slice(-1)[0]);
    // console.log('정보확인666 : ', res_result.data.slice(-1)[0].complex_id);

    if (res_result) {
      // console.log('res_result:::',res_result);
      // setComplex_searchlist(res_result.result);
      setComplex_searchlist(res_result.data);
      setlastindex(res_result.data.slice(-1)[0]);
    }
    console.log('동작5');
  }
  ...
  const infinitScroll =async () => {
    ...
    let res_result = await serverController.connectFetchController(`/api/complexes?auto_complete=1&filter=${JSON.stringify({ default: { bld_type: activeIndex == 0 ? '아파트' : '오피스텔' }, special: { search: keyword } })}&before_complex_id=${lastindex.complex_id}`, 'GET');
    await setComplex_searchlist([...complex_searchlist, ...res_result.data])

    console.log('9999', lastindex.complex_id);
    console.log('9999', res_result);
    console.log('9999', activeIndex);
    console.log('9999', keyword);
    console.log('9999', lastPostion);
    console.log('동작3', lastPostion);
  }
  ...
  const openOption = (v) =>{
    ...
    if (!!v){
      console.log('확인456', v); 
      setSelect_complexid(v.complex_id); 
      setSelectInfo(true);
    } else if (v == null){
    // setSelect_complexid('');
    setSelectInfo(false);
  ...
  const filterOptions = createFilterOptions({
    stringify: (option) => `${option.complex_name} ${option.addr_jibun} ${option.addr_road}`
  });
    ...
    <>
      <Autocomplete
        ...
        ListboxProps={{
            ...
            // console.log('동작1');
            // console.log('동작2', e.currentTarget);
            // console.log('동작5', e.currentTarget.scrollTop);
            // console.log('동작6', listboxNode);
            // console.log('동작7', listboxNode.scrollHeight);
            ...
            if (listboxNode.scrollTop + listboxNode.clientHeight === listboxNode.scrollHeight) {
              ...
              //     behavior: 'smooth'
              //   })
              // listboxNode.scrollTop = 1000
            }
            
            // listboxNode.scrollTo(0, 1000)
            console.log('동작4');
            }
        }}
        //!!@@ 211129_이형규> onClick으로 처리하면 키보드 컨트롤을 인식못하므로, onChange에서 setSelect_complexid()호출하였음. But v.complex_id 가 없는 경우 에러발생하므로, 그런경우 setSelectInfo(false)분기처리해줘야 함
        // onChange={(e, v) => {setSelect_complexid(v.complex_id); setSelectInfo(true)}}
        options={complex_searchlist}
        autoHighlight
        filterOptions={filterOptions}
        getOptionLabel={(option) => option.complex_name }
