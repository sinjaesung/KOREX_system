    ...
    return (
      <Container>
          <Li>
            ...
            <Infos>
              <ConditionDiv>
                <ConditionType color={type}>{value.product_create_origin}</ConditionType>&nbsp;&nbsp;상태 : <Condition>{value.txn_status}</Condition>
              </ConditionDiv>
              
                {
                  /*사실상 거래개시상태인 것들만 가능하다*/
                  value.prd_exculsive_status==1 && value.prd_exculsive_start_date && value.prd_exculsive_end_date ?
                  <TopBox>
              ...
              <Address>
                <Left>의뢰인명</Left>
                <Right>{value.request_man_name}{value.request_mem_phone}({value.request_mem_selectsosokid})</Right>
              </Address>
            </Infos>
            <RightMenu>
              <Alarm>
                            ...
                            if (value.product_create_origin == '중개의뢰') {
                              ...
                              if (value.request_mem_phone && value.request_man_name) {
                                let sms_info = {
                                  ...
                                  msg: value.prd_identity_id + '::해당 매물 선임중개사에서 거래개시승인요청이 왔습니다.\n\n' + maemul_info_loca + '\n\n [[내용확인]]:' + "https://korexpro.com/Preview/" + value.prd_identity_id,
                              ...
                              if(prdstatus_result.success){
                                ...
                                if(value.product_create_origin=='중개의뢰'){
                                  ...
                                  if(value.request_mem_phone && value.request_man_name){
                                    let sms_info = {
                                      receiver: value.request_mem_phone,
                                      msg:value.prd_identity_id+'::해당 매물 선임중개사에서 거래완료승인요청이 왔습니다.\n\n[[내용확인]]:https://korexpro.com/PreviewComplete/'+value.prd_identity_id+'\n\n'+maemul_info_loca,
                                      msg_type:'LMS',
                                      title:'거래완료승인요청 전송',
                                      type:'거래완료승인요청'
                                    };
