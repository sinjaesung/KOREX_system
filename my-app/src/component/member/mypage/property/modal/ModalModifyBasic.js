...
//필터 모달
export default function ModifyBasic({dataRef, timeRef, dayRef, holiRef, load_day_select_count, load_is_tour_holiday_except,load_tour_set_days,load_tour_set_times}) {
    console.log('==>>>>>modalAddbasic요소 실행>>>>>>>>>>',load_day_select_count,load_is_tour_holiday_except,load_tour_set_days,load_tour_set_times);

    const [isHoli, setIsHoli] = useState(true);
   ...
   // 아래 코드는 사용하지 않았습니다. 수정용 페이지에선 조회떄에 데이터를 초기데이터 보여주는 부분으로 인해 state지정도 필요함.
   useEffect(() => {
    setNormal_select_days(load_tour_set_days);
    setNormal_select_times(load_tour_set_times);
    setNormal_isholidayexcept(load_is_tour_holiday_except);
    setNormal_select_daycount(load_day_select_count);
                    ...
                    dateData.map((item, index) => {
                      return(
                        <InBox key={index}>
                          ...
                          <InputCheck type="checkbox" onChange={() => change_normal_days()} className='normal_select_days'name="" id={item.eng} value={item.eng} checked={ normal_select_days.indexOf(item.eng)!=-1}/>
                      ...
                      timeData.map((item, index) => {
                        return(
                          <LineBox key={index}>
                            ...
                            <InputCheck type="checkbox" onChange={() => change_normal_days_times()} className='normal_select_times' name="" id={item.eng} value={item.kor} checked={normal_select_times.indexOf(item.kor)!=-1}/>
                    ...
                    <AddSelectConditionList ref={dayRef}> 
                      <InOption selected disabled>갯수 선택</InOption>
                      {
                        elData.map((item, index) => {
                          return(
                            <InOption value={item} key={index} selected={item==normal_select_daycount}>{item}</InOption>
                          )
                        })
                      }
                ...
                <AddBox>
                  ...
                  <AddSelectCondition>
                    <LineBox>
                      ...
                      <InputCheck ref={holiRef} type="checkbox" name="" checked={normal_isholidayexcept} id="hday" onClick={change_isholidayexcept}/>
