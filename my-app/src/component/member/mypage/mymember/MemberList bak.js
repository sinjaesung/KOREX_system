  ...
  const handleClose = () => {
    ...
    return (
        <Container>
            <MemberList>
              <ProfileImg>
                <Img src={(value.mem_img=='' || !value.mem_img)?Profile:globe_aws_url+value.mem_img} alt="프로필이미지"/>
              </ProfileImg>
              <MemberInfo>
                <Name>{value.user_name}</Name>
                <Grade>{value.cm_type==1?'관리자':'팀원'}</Grade>
                <Phone>{value.phone}</Phone>
                <RegiDate>{value.create_date}</RegiDate>
              </MemberInfo>
          ...
          >
            <MenuItem>
              <Link to={"/MyMemberEdit/" + value.mem_id} className="data_link" />수정</MenuItem>
            <MenuItem>
