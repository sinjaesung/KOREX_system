...
export default function InputForm_ItemSpec_Basic({ mode, failModal, nextModal, forwarding, brokerRequest_product, saveNext, setModalOption, modalOption, prd_id, updateModal, serveruploadimgs_server, changeaddedimgs_server, prd_state }) {
  ...
  const [imgsfilse, setimgsfilse] = useState([])

  useEffect(() => {
    console.log('이미지 파일 확인', serveruploadimgs_server);
    console.log('이미지 파일 확인', serveruploadimgs_server.length);
    console.log('이미지 파일 확인', changeaddedimgs_server);
    console.log('이미지 파일 확인', changeaddedimgs_server.length);
    // console.log('이미지 파일 확인', changeaddedimgs_server.split(',').length);
    // console.log('이미지 파일 확인', changeaddedimgs_server);
    // console.log('이미지 파일 확인', changeaddedimgs_server.split(','));
    setimgsfiles(changeaddedimgs_server.length);
    
    // if(mode == '수정'){
    //   if (!!serveruploadimgs_server){
    //     setimgsfiles(serveruploadimgs_server.length);
  ...
  useEffect(async () => {
    if (mode == '수정') {
      ...
      console.log(' 여기를 확인112', !!brokerRequest_product.prd_imgs ? brokerRequest_product.prd_imgs.split(',').length : '')
  ...
  }, [exclusive_periods, buildingtype, Selladdress, Sellstate, maemulname, jeonyongdimension, jeonyongpyeong, supplydimension, supplypyeong, selltype, sellprice, monthsellprice, Managecost, ismanagementcost, viewDate, managecostincludes, ibju_specifydate, usetype, isstorejob, storejob, isrightprice, imgsfiles])
  ...
  const nextbtn2 = async () => {
    ...
    let body = {
      ...
      prd_imgs: serveruploadimgs_server.length !== 0 ? brokerRequest_product.prd_imgs.concat(',', res2.data.join(',')) : brokerRequest_product.prd_imgs,
      prd_price: sellprice,//	매매금, 전세금
      include_managecost: managecostincludes.join(','),//관리비포함항목
    ...
    console.log('수정확인555', body );
    console.log('수정확인555', serveruploadimgs_server.length );
  ...
  const saveBtn = async () => {
    ...
    let res2 = await serverController2.connectFetchController('/api/images', 'POST', formData);
    setimgsfilse(res2.data)
    console.log('기본정보확인', serveruploadimgs_server);



    // console.log('기본정보확인', res2.data);
    ...
    let body = {
      ...
      exclusive_periods: exclusive_periods,	//전속기간(개월)
      prd_imgs: serveruploadimgs_server.length !== 0 ? brokerRequest_product.prd_imgs.concat(',', res2.data.join(',')) : brokerRequest_product.prd_imgs ,
      // prd_imgs: !!imgsfilse ? brokerRequest_product.prd_imgs.concat(',', imgsfilse.join(',')) : brokerRequest_product.prd_imgs,
      prd_price: sellprice,//	매매금, 전세금
      include_managecost: managecostincludes.join(','),//관리비포함항목
      is_managecost: ismanagementcost,	//관리비여부
    ...
    console.log('수정확인555', serveruploadimgs_server.length);
    // console.log('수정확인555', serveruploadimgs_server);
    // console.log('수정확인555', brokerRequest_product.prd_id);
    console.log('수정확인555', brokerRequest_product.prd_imgs);
    console.log('수정확인555', !!brokerRequest_product.prd_imgs ?brokerRequest_product.prd_imgs.concat(',', res2.data.join(',')) : res2.data.join(','));
    //####수정 api 동작이 false됨 body를 모두 빈값으로 넣어 돌렸는데도 false가 나타남..... 
    let patchdata = await serverController.connectFetchController(`/api/products/${brokerRequest_product.prd_id}`, 'PATCH', JSON.stringify(body));
    console.log('수정확인555', patchdata);
              ...
              <div className="par-spacing">
                {
                  mode == '수정' ?
                    <MUTextField_100 required label={`${selltype}가`} placeholder={`${selltype}가 입력`} variant="standard" value={sellprice}
                      onChange={selltype == '월세' ? change_monthsellprice : change_sellprice}  InputProps={{ endAdornment: (<InputAdornment position="end">만원</InputAdornment>) }} />
                    :
                    <MUTextField_100 required label={`${selltype}가`} placeholder={`${selltype}가 입력`} variant="standard" value={sellprice} onChange={selltype == '월세' ? change_monthsellprice : change_sellprice}
                      InputProps={{ endAdornment: (<InputAdornment position="end">만원</InputAdornment>) }} />
            ...
            {/* 거래유형 > 월세 선택됐을때 아래 내용 나와야함 */}
            {
              ...
              <div className="par-spacing">
                {mode == '수정' ?
                  ...
                  <div className="flex-spabetween-center">
                    <MUTextField required label="보증금" placeholder="보증금 입력" variant="standard" value={sellprice} onChange={change_sellprice}
                      InputProps={{ endAdornment: (<InputAdornment position="end">만원</InputAdornment>) }} />
                    <Divider sx={{ height: 28, mx: 1 }} orientation="vertical" />
                    <MUTextField required label="월세" placeholder="월세" variant="standard" value={monthsellprice} onChange={change_monthsellprice}
                      InputProps={{ endAdornment: (<InputAdornment position="end">만원</InputAdornment>) }} />
                  </div>
                  :
                  <div className="flex-spabetween-center">
