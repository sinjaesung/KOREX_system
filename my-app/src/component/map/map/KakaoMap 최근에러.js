  ...
  async function getProduct_idleCase() {
    ...
    if (mapData) {
      ...
      if (mapData.level >= 1 && mapData.level <= 5) {
        console.log("--------------------------------------1111111111111111111-----------------1----------------------------------");
        updateProduct();
    ...
    if (array) {
      array.map(item => {
        ...
        if (mapData.level < cluLevel) {
          ...
          let recent_transaction_year_shorthand = item.contract_ym.substr(2, 2);
          let transaction_date_shorthand = (recent_transaction_year_shorthand + '.' + recent_transaction_month + '.' + item.contract_dt);
          /*최근거래일날짜기준값이
          ...
          */
          ...
          //create_content.innerHTML = `<p class="type2">` + item.contract_ym + item.contract_dt + `</p><p class="type1">` + item.type + item.deposit + `</p>`;
          create_content.innerHTML = `<span class="date">` + transaction_date_shorthand + `</span><span>` + item.type + `</span><span>` + item.deposit + `</span>`;
  ...
  useEffect(() => {
    ...
    if (mapRightRedux.mapStyle == "roadView") {
      ...
      function toggleRoadview(position) {
        ...
        console.log(roadViewRefsss);
        console.log("pl;z");
        console.log(mapRightRedux.mapStyle);
        ...
        rvClient.getNearestPanoId(position, 50, function (panoId) {
          ...
          if (panoId === null) {
            if (roadViewRef.current) {
              roadViewRef.current.style.display = 'none';
              roadViewRefsss.current.style.display = 'none';
              rvWrapperRef.current.style.pointerEvents = 'none';
            }
            if (roadViewModalRef.current) {
              roadViewModalRef.current.style.display = 'none';
            ...
            if (roadViewRef.current) {
              ...
              roadViewRefsss.current.style.display = 'block';
  ...
  return (
    <SuperContainer>
      ...
      */}
      <Landscape>
        {
            mapRightRedux.mapStyle === "roadView" ? 
            <RoadViewModalDiv ref={roadViewModalRef}>
            <DraggableResizeModal
              title={'거리뷰'}
              open={true}
