...
export default function SideItemDetail({ openBunyang, rank, updatePageIndex, setHistoryInfo, historyInfo, report, setReport, reser, updateReserveModal, click_prdidentityid, mode, Previewinfo, previewprobrokerinfo, prdid, test }) {
  ...
  console.log('아이템999_매물정보', Previewinfo);
  console.log('821120___previewprobrokerinfo', previewprobrokerinfo);
  ...
  const datainsert = async () => {
    if (mode == "의뢰인동의뷰" || mode == "미리보기") {
      ...
      console.log('옵션 유무확인', Previewinfo);


      // 물건
      if (!Previewinfo.floorint && !Previewinfo.grd_floor && !Previewinfo.supply_area && !Previewinfo.exclusive_area && !Previewinfo.room_count && !Previewinfo.bathroom_count && !Previewinfo.direction && !Previewinfo.entrance && !Previewinfo.heat_method_type && !Previewinfo.heat_fuel_type) {
        setoption1(false);
      } else {
        setItem([
          {
            title: "해당층/총층", desc: !!Previewinfo.floorint || !!Previewinfo.grd_floor ?
              ...
              !!Previewinfo.floorint && !Previewinfo.grd_floor ?
                Previewinfo.floorint + '층 /' + '정보없음' : '정보없음' + '/' + Previewinfo.grd_floor + '층' : null
          },
          ...
          { title: "방/욕실 수", desc: !!Previewinfo.room_count || !!Previewinfo.bathroom_count ? Previewinfo.room_count + '/' + Previewinfo.bathroom_count : null },
          ...
          {
            title: "난방", desc: !!Previewinfo.heat_method_type || !!Previewinfo.heat_fuel_type ?
              !!Previewinfo.heat_method_type && !Previewinfo.heat_method_type ?
                Previewinfo.heat_fuel_type + '/' + '선택없음' : '선택없음' + '/' + Previewinfo.heat_method_type : null
          },
        ])
      }
      // 거래
      // if (!Previewinfo.managecost && !Previewinfo.include_managecost && !Previewinfo.isconractrenewal && !Previewinfo.loanprice && !Previewinfo.existed_deposit && !Previewinfo.existed_month_price && !Previewinfo.is_immediate_ibju){
      //   setoption2(false);
      // }else{
      setDeal([
        { title: "관리비", desc: Previewinfo.managecost == 0 ? '없음' : '있음' },
        ...
        { title: "융자금", desc: !!Previewinfo.loanprice ? Previewinfo.loanprice + '만원' : null },
        { title: "기보증금", desc: !!Previewinfo.existed_deposit ? Previewinfo.existed_deposit + '만원' : null },
        { title: "월세", desc: !!Previewinfo.existed_month_price ? Previewinfo.existed_month_price + '만원' : null },
      ])
      // }

      // 옵션
      if (!Previewinfo.space_option) {
        setoption3(false);
      } else {
        setOption([
      ...
      if (Previewinfo.prd_type == '아파트' || Previewinfo.prd_type == '오피스텔') {
        if (!Previewinfo.approval_date && !Previewinfo.household_cnt && !Previewinfo.total_parking_cnt) {
          setoption5(false);
        } else {
          setDanji([
      ...
      // // 매물설명  
      if (!Previewinfo.prd_description_detail) {
        setoption4(false);
        if (!!Previewinfo.prd_description) {
          setDesc([
            [
      ...
      // 위치
      console.log('지도 좌표', Previewinfo.prd_latitude);
      console.log('지도 좌표', Previewinfo.prd_longitude);
      ...
      console.log('지도 물건 정보', productsinfoitem);
      // 물건
      if (!productsinfoitem.data.floorint && !productsinfoitem.data.grd_floor && !productsinfoitem.data.supply_area && !productsinfoitem.data.exclusive_area && !productsinfoitem.data.room_count && !productsinfoitem.data.bathroom_count && !productsinfoitem.data.direction && !productsinfoitem.data.entrance && !productsinfoitem.data.heat_method_type && !productsinfoitem.data.heat_fuel_type) {
        setoption1(false);
      } else {
        setItem([
          {
            title: "해당층/총층", desc: !!productsinfoitem.data.floorint || !!productsinfoitem.data.grd_floor ?
              ...
              !!productsinfoitem.data.floorint && !productsinfoitem.data.grd_floor ?
                productsinfoitem.data.floorint + '층 /' + '정보없음' : '정보없음' + '/' + productsinfoitem.data.grd_floor + '층' : null
          },
          ...
          { title: "방/욕실 수", desc: !!productsinfoitem.data.room_count || !!productsinfoitem.data.bathroom_count ? productsinfoitem.data.room_count + '/' + productsinfoitem.data.bathroom_count : null },
          ...
          {
            title: "난방", desc: !!productsinfoitem.data.heat_method_type || !!productsinfoitem.data.heat_fuel_type ?
              !!productsinfoitem.data.heat_method_type && !productsinfoitem.data.heat_method_type ?
                productsinfoitem.data.heat_fuel_type + '/' + '선택없음' : '선택없음' + '/' + productsinfoitem.data.heat_method_type : null
          },
        ])
      }
      // 거래
      // if (!productsinfoitem.data.managecost && !productsinfoitem.data.include_managecost && !productsinfoitem.data.isconractrenewal && !productsinfoitem.data.loanprice && !productsinfoitem.data.existed_deposit && !productsinfoitem.data.existed_month_price && !productsinfoitem.data.is_immediate_ibju){
      //   setoption2(false);
      // }else{
      setDeal([
        ...
        { title: "융자금", desc: !!productsinfoitem.data.loanprice ? productsinfoitem.data.loanprice + '만원' : null },
        { title: "기보증금", desc: !!productsinfoitem.data.existed_deposit ? productsinfoitem.data.existed_deposit + '만원' : null },
        { title: "월세", desc: !!productsinfoitem.data.existed_month_price ? productsinfoitem.data.existed_month_price + '만원' : null },
      ])
      // }

      // 옵션
      if (!productsinfoitem.data.space_option) {
        setoption3(false);
      } else {
        setOption([
          { title: "공간", desc: productsinfoitem.data.space_option },
      ...
      if (productsinfoitem.data.prd_type == '아파트' || productsinfoitem.data.prd_type == '오피스텔') {
        if (!productsinfoitem.data.approval_date && !productsinfoitem.data.household_cnt && !productsinfoitem.data.total_parking_cnt) {
          setoption5(false);
        } else {
          setDanji([
      ...
      // // 매물설명  
      if (!productsinfoitem.data.prd_description_detail) {
        setoption4(false);
        if (!!productsinfoitem.data.prd_description) {
          setDesc([
            [
  ...
  // console.log('821120___product', product);
  // console.log('821120___deteilproducts', deteilproducts);
  console.log('821120___broker', broker);

  {/*!!@@211125_이형규> 에러--- 콘솔에서는 num값 체크하지만, 화면에서 아래와 같은 에러뜸. 그래서 더이상 진행안함.
    Unhandled Rejection (TypeError): Cannot read properties of null (reading 'childElementCount')
  */}
  const countChdNum = async (a) => {
    // var el = document.getElementById(a);
    // var num = el.childElementCount;
    // console.log('넘넘넘____', num);
  }
      ...
      <Sect_Article_1>
        <div className="pd-0p75">
          <div className="par-spacing">
            {/*!!@@211125_이형규> 에러--- 확인요청프리뷰말고 지도물건상세에서는 broker데이터 못불러옴.*/}
        ...
        <div className="pd-0p75" >
          ...
          <div className="par-spacing-after">
            <div className="flex-spabetween-center">
              {/* <span className="capt-00">!!!등록번호안나옴 {product.number}</span> */}
          ...
          <div className="par-spacing-after">
            {
              Previewinfo.prd_status === ("거래개시동의요청" || "거래준비") ?
                <p className="list-tit fw-b c-secondary">전속기간: {Previewinfo.exclusive_periods}개월</p>
                :
          ...
          <div className="par-spacing-after">
            ...
            <div>
              <span className="list-tag vAlign-m">[{Previewinfo.prd_type}]</span>&nbsp;
              <span className="capt-a1 vAlign-m">{Previewinfo.addr_jibun}</span>
            </div>
            
            <p className="detail-subtit fw-b vAlign-m">{Previewinfo.prd_name}</p>
          ...
          <div className="par-spacing-before">
            <p className="capt-a1">{Previewinfo.prd_description}</p>
          </div>
        </div>
        <div className="divider-a1" />
        ...
        <div>
          ...
          {option1 ?
            <MUAccordion>
              <AccordionSummary
                ...
                id="panel1a-header"
              >
                <Typography>물건</Typography>
              </AccordionSummary>
              <AccordionDetails >
                <ul id="a-item" abc={countChdNum("a-item")}>

                  {
                    item.map((item, index) => {
                      if (!!item.desc) {
                        return (
                          // <ListEl key={index} title={item.title} desc={item.desc} descM={item.descM && item.descM} ChangeM={item.ChangeM && item.ChangeM} isVal={item.isVal} />
                ...
                <ul>
                  {
                    deal.map((item, index) => {
                      if (!!item.desc) {
                        return (
                          <ListEl key={index} title={item.title} desc={item.desc} ChangeM={item.ChangeM && item.ChangeM} isVal={item.isVal} />
                        )
                      }
                    })
                  }
            ...
            option4 ?
              <MUAccordion>
                <AccordionSummary
                  ...
                  id="panel2a-header"
                >
                  <Typography>매물설명</Typography>
                </AccordionSummary>
                <AccordionDetails>
                  {/* <div className="capt-a1">{Previewinfo.prd_description_detail}</div> */}
                  <TextArea>{!!desc[1] ? desc[1] : '설명없음'}</TextArea>
                </AccordionDetails>
              </MUAccordion>
              :
              null
          ...
          <MUAccordion defaultExpanded>
            ...
            <AccordionDetails>
              <div className="flex-spabetween-center">
                <p className="capt-a1">{isAddress ? position.address : position.roadAddress}</p>
              ...
              </div>
              {
                Object.keys(position).length === 0 ?
                  ...
                  :
                  <MapArea>
                    {console.log('지도좌표', position.lat)}
                    {console.log('지도좌표', position.lng)}
...
const MUAccordion = styled(Accordion)``
