          ...
          <>
            {
              productRedux.probroker.map((value, index) => {
                ...
                console.log('81_____________',value);
                return (
                  <TabContent key={index} aniDelay={animationDelay(index)}>
                    <Link onClick={() => onClickEl(value)} className="data_link"></Link>
                    <MUStack direction="row" spacing={1}>
                      {
                        permission_info && permission_info['pro_apt_id'] ?
                          // <Tag>{'아파트'+permission_info['apt_name']}</Tag>
                          <MUChip label={'아파트' + permission_info['apt_name']} />
                          :
                          null
                      }
                      {
                        permission_info && permission_info['pro_oft_id'] ?
                          // <Tag>{'오피스텔'+permission_info['oft_name']}</Tag>
                          <MUChip label={'오피스텔' + permission_info['oft_name']} />
                          :
                          null
                      }
                      {
                        permission_info && permission_info['is_pro_store'] ?
                          // <Tag>상가</Tag>
                          <MUChip label={'상가'} />
                          :
                          null
                      ...
                      {
                        permission_info && permission_info['is_pro_office'] ?
                          // <Tag>사무실</Tag>
                          <MUChip label={'사무실'} />
                          :
                          null
                    ...
                    <BottomBox>
                      <LeftContent>
                        <ItemInfo>
                          ...
                          <Address>{probroker_info.addr_road}</Address>
                          <SellList>
                            <List>매매 <ColorOrange>{ongoing_maemae_productscnt}</ColorOrange></List>
                            <Part />
                            <List>전세 <ColorOrange>{ongoing_jeonse_productscnt}</ColorOrange></List>
                            <Part />
                            <List>월세 <ColorOrange>{ongoing_walse_productscnt}</ColorOrange></List>
                          </SellList>
                        </ItemInfo>
                      </LeftContent>
                      <RightContent>
