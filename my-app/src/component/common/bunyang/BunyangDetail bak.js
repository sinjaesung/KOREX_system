...
export default function BunyangDetail({ bunyangDetail, updatePageIndex, setLive, setDetailImg, setCal, BunyangDate, clickId, setImgArr, imgArr }) {
  ...
  useEffect(async()=>{
    ...
    serverController.connectFetchController(`/api/bunyang/reservation/setting?bp_id=${bunyangDetail.bp_id}&tour_type=3`, 'GET', null, function (res) {
      if (res.success == 1) {
  ...
  return (
    <>
      <Sect_R2>
        <BunyangDetailCont bunyangDetail={bunyangDetail} onClickList={onClickList} setLive={setLive} setDetailImg={setDetailImg} setCal={setCal} livetour={livetour} optionList={optionList} numberWithCommas={numberWithCommas} contact={contact} livetour={livetour}invitetour={invitetour} active={active} LikeBtnEvent={LikeBtnEvent} />
      </Sect_R2>
    </>
  )
}
