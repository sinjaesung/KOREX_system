...
export default function ModalCalendar({bunyangDetail,cal, setCal}){
  ...
  useEffect(() => {  
    ...
    serverController.connectFetchController(`/api/bunyang/reservation/setting?bp_id=${bunyangDetail&&bunyangDetail.bp_id}&tour_type=3`,'GET',null,function(res){
      if(res.success == 1){
  ...
  const pageLoader = () =>{
    switch (pageIndex) {
      case 0: return <ModalCalendarFirst holidayMap={holidayMap} changeMonth={changeMonth} tour={tour} updatePageIndex={updatePageIndex} offModal={offModal} setModalOption={setModalOption} cal={cal} setCal={setCal} SelectDate={SelectDate} setSelectDate={setSelectDate}/>;
      case 1: return <ModalCalendarSecond tour={tour} selectTime={selectTime} updatePageIndex={updatePageIndex} cal={cal} setCal={setCal} SelectDate={SelectDate} setSelectTime={setSelectTime} checkedTime={checkedTime} setcheckedTime={setcheckedTime}/>
      case 2: return <ModalCalendarThird tour={tour} userList={userList} setUserList={setUserList} offModal={offModal} setModalOption={setModalOption} finalModal={finalModal} confimReservation={confimReservation} updatePageIndex={updatePageIndex} cal={cal} setCal={setCal} />
      default: return <ModalCalendarFirst holidayMap={holidayMap} changeMonth={changeMonth}  tour={tour} updatePageIndex={updatePageIndex} cal={cal} setCal={setCal} SelectDate={SelectDate} setSelectDate={setSelectDate}/>;
    }
  }
  ...
  const confimReservation = () =>{
    ...
    let data = {
      ...
      td_id : tour.td_id,//신청하려는 tourid(표면상 노출된 가장 최근등록된것 다수개가 아님 플젝별 하나씩의 방문예약셋팅)에 대한 처리.tour.tourid,td_id 언제기간/언제시간대로 되어있던분양프로젝트 플젝셋팅에 대한 요청인건지.
      userList : userList,
      ...
      company_id : userInfo.company_id?userInfo.company_id : null,//소속 중개사companyid값 어떤 소속중개사인상태에서 신청한 분양 방문or라이브예약인건지
      tr_type : 1,
    ...
    serverController.connectFetchController(`/api/bunyang/reservation`,'POST',JSON.stringify(data),async function(res){
      if(res.success == 1){
        ...
        let noti_infoss={
          ...
          //개별적으로 분양방문예약신청한 중개의 당시 소속 어떤소속중개사상태에서 신청했던건지 구분위함.
          sosok_company_id:userInfo.company_id? userInfo.company_id : -1,//어떤 companyid상태에서 신청했던
          action:'update',
          type:'private'
        }
