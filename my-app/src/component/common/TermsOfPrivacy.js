...
export default function ModalTest2({ termprivacy, openTermPrivacy }) {
  ...
  useEffect(async () => {
    let termsTemp = await serverController.connectFetchController('/api/terms?terms_type=1','GET');
    setTermsList(termsTemp.data);
    setTerms(termsTemp.data[0]);
  }, [])

 const getTermsDetail = async (id) =>{
    let termsTemp = await serverController.connectFetchController(`/api/term?terms_type=1&terms_id=${id}`,'GET');
    setTerms(termsTemp.data[0]);
  }
  
//모달창
  if(termprivacy == false)
    return null;
    return (
      <Container>
        <MUModal
          onClose={() => { openTermPrivacy(false) }}
          open={termprivacy}
          maxWidth={'md'}
          fullWidth={true}
        >
          <BootstrapDialogTitle id="customized-dialog-title" onClose={() => { openTermPrivacy(false) }}>
            개인정보 처리방침
            &nbsp;&nbsp;
        ...
        {/* <ModalBg onClick={()=>{openTermPrivacy(false)}}></ModalBg>
        <ModalContent>
          <ModalClose>
            <Link onClick={()=>{openTermPrivacy(false)}}>
              <CloseImg src={CloseIcon}/>
            </Link>
          </ModalClose>
          <ModalTop>
            <Title>개인정보 처리방침</Title>
            <select className="term1" onChange={(e)=>{
              console.log('선택약관 value:',e.target.value);
              setSelectTerms(terms_content[e.target.value]);
            }}>
              <option value="" disabled selected>이전 개인정보 처리방침 보기</option>
