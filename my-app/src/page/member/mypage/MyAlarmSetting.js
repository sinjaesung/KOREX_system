...
import AlarmSetting from '../../../component/member/mypage/alarm/AlarmSetting';
      ...
      if (res.login_session == null) {
        ...
        if (user_info.success) {
          ...
          let body_infoss = {
            ...
            company_id: user_info.user_data.company_id,//기업,중개사였다면 회사소속companyid값(선택되어있던 companyid값)
            bp_id: bunyangTeam.bunyangTeam.bp_id,//선택한 분양대행사의 bpid값 
          }
          let resss = await serverController.connectFetchController('/api/alram/alramSetting_status', 'POST', JSON.stringify(body_infoss));

          if (resss) {
            console.log('resssss alramsettings:', resss);
            if (resss.success) {
              let result = resss.result;
              if (result) {
                setalramsetting(result);
              }
            } else {
              alert(resss.message);
            }
  ...
  return (
    <div className="flex-col-spabetween minHgt-100vh">
      <div>
        ...
        <AlarmSetting setalramsetting={setalramsetting} alramsetting={alramsetting} />
      </div>
      <CommonFooter />
