...
export default function MyLivePage() {
  ...
  async function updateList() {
    let result = await serverController.connectFetchController(
      `/api/bunyang/my/reservation?mem_id=${userInfo.memid}&company_id=${userInfo.company_id}&tr_type=${2}&order=${filter.order}&sort=${filter.sort}`
      , "GET"
      , null
    );
    setList(result.data);
  ...
  const updateModal = () => {
    ...
    setModalOption({
      ...
      setShow: offModal,
      title: "필터",
      content: { type: "components", text: `Testsetsetsetsetestse`, component: <ModalFilterComponent filter={filter} setFilter={setFilter} /> },
  ...
  return (
    <div className="flex-col-spabetween minHgt-100vh">
      ...
      */}
        ...
        <MyLive list={list} setList={setList} updateModal={updateModal} updateList={updateList} alramsetting_tiny={alramsetting_tiny} setalramsetting_tiny={setalramsetting_tiny} />
      </div>
      <CommonFooter />
      {/*
