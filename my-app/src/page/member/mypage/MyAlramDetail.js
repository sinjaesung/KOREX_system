...
import MyAlramDetail from '../../../component/member/mypage/alarm/MyAlramDetail';
...
export default function Join({ match }) {
  ...
  var noti_id = match.params.id;
  const [alramDetailinfo, setalramDetailinfo] = useState([]);
  // //이용약관
  ...
  useEffect(async () => {
    ...
    let alramdetail_result = await serverController.connectFetchController('/api/alram/myalram_detail', 'POST', JSON.stringify(body_info));
    if (alramdetail_result) {
      console.log('알람상세 결과 :', alramdetail_result);

      if (alramdetail_result.success) {
        if (alramdetail_result.result) {
          setalramDetailinfo(alramdetail_result.result);
        }
      }
    }
  }, []);
  ...
  return (
    <div className="flex-col-spabetween minHgt-100vh">
      <div>
        ...
        <MyAlramDetail alramDetailinfo={alramDetailinfo} />
      </div>
      <CommonFooter />
